...
export function useGetRandomBonusSymbols() {
      ...
      if (unlockedSupers.length > 0) {
        ...
        for (let i = 0; i < elements.length; i++) {
          if (unlockedSupers.includes(elements[i])) {
            ...
            if (improved) {
              elements[i] =
                getCorrespondingSuperSymbol(elements[i]) ?? elements[i];
...
export function useGetRandomDevilDeals() {
  const canDevilDealBeInShop = useCanDevilDealBeInShop();
  return useCallback(() => {
